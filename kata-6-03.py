...
given a dictionary of customers, a minimum number of orders and a minimum order value, returns a set of the customers
...
def pizza_rewards(customers, min_orders, min_price):
    return {customer for customer in customers.keys() if len([price for price in customers[customer] if price >= min_price]) >= min_orders}
